...
deny contains msg if {
    data.budget.max_monthly_increase
    ...
    increase > data.budget.max_monthly_increase
...
deny contains msg if {
    data.services.max_per_service
    service := input.cost_by_service[service_name]
    limit := data.services.max_per_service[service_name]
    service > limit
